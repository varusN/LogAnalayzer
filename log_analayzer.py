...
def main():
    ...
    logger.info(f"Start with config: REPORT_SIZE: {report_size}, REPORT_DIR: {report_dir}, LOG_DIR: {log_dir}, APP_LOG: {app_log}")
    ...
    try:
        ...
        check_parser_errors(log_records, log_file, config['parser_errors_threshold'])
    except Exception:
...
def check_parser_errors(log_records, log_file, parser_errors):
    if log_records == 0:
        logger.error(f"Empty file {log_file}, exit")
        sys.exit()
    try:
        if 1-parsed_records/log_records > parser_errors:
...
def check_extention(file):
    ...
    if extension not in ('.gz', '.log'):
        logger.error(f"Incorrect extension in file {file}, .gz and .log are supported only, exit")
        sys.exit()
